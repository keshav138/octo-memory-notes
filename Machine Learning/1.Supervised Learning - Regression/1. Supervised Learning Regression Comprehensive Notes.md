...
Logistic regression is called a "regression" algorithm ==because its underlying mathematical engine is fundamentally identical to linear regression==. It calculates the exact same linear combination of inputs and weights (y = b₀ + b₁x₁ + ...), but then passes that continuous numeric output through a logistic (sigmoid) function to predict a probability between 0 and 1. 

A sigmoid function is ==a bounded, differentiable mathematical function with a characteristic "S"-shaped curve==
...
---
## Log Loss (Cross-Entropy Loss)  - Cost Function For Logistic Regression

Log loss measures the performance of a classification model whose output is a probability value between 0 and 1. It punishes incorrect predictions with a high penalty, meaning that a prediction of 0.99 when the true label is 0 results in a massive loss. [1, 2, 3, 4, 5]

The formula for a single data point is:  
$$L = - \left[ y \log(\hat{y}) + (1 - y) \log(1 - \hat{y}) \right]$$

For an entire dataset of N samples, it calculates the average of these losses:  
$$J = - \frac{1}{N} \sum_{i=1}^{N} \left[ y_i \log(\hat{y}_i) + (1 - y_i) \log(1 - \hat{y}_i) \right]$$

Where:

- N = the number of samples
- \(y_{i}\) = the actual class label for the \(i^{th}\) sample (0 or 1)
- \(\^{y}_{i}\) = the predicted probability that the sample belongs to class 1 
- Convexity: Unlike Mean Squared Error, Log Loss provides a convex curve for logistic regression, guaranteeing that optimization algorithms can find the single lowest point.
- Interpretation: Minimising Log Loss directly maximizes the accuracy of your model's predicted probabilities. 

---

## Gradient Descent

Gradient descent is an optimization algorithm used to minimize a cost function by iteratively moving in the direction of the steepest descent. It acts like a hiker trying to find the bottom of a valley in heavy fog by feeling the slope of the ground under their feet. 

- The Gradient: This is a vector of partial derivatives that points toward the direction of the steepest increase; taking a negative step moves you downward.
- Learning Rate (α): This hyperparameter defines the size of the steps taken toward the minimum, where a rate that is too high can overshoot the target and a rate that is too low makes learning inefficient. 
...
A **linear relation** means that ==two variables change together at a constant rate==. If you graph the data points, they form a straight line. 
