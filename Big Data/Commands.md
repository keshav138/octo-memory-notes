...
19. `hdfs dfs -getmerge <directory> file:///<filepath>/<filename>` - ==to merge local files in a directory in Hadoop==
20. `hdfs dfs -chmod +[permission] <filePath>` - ==change the permission of a file -- -chmod ugo+xxx, stands for user, group and owner, xxx is write, write, write permission==
21. `hdfs dfs chown <ownerName>`
22. `hdfs dfs chgrp <groupName>`
23. `hdfs dfs chown <ownerName>:<groupName>` - ==change the owner and group for a file/folder one shot==
24. `hdfs dfs -test -e <filePath>` - ==runs a test to check if a file exists or not== 
...
27. `hdfs dfs -checksum` - ==gives you the file checksum and the algorithm used for its hashing==
